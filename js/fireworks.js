// 1. 生成星星背景
      ...
      class Firework {
        ...
        // 炸开生成粒子（替换十字形为螺旋形，共5种自然绚烂样式）
        explode() {
          const particleCount = 150; // 粒子数量增加，更绚烂
          ...
          for (let i = 0; i < particleCount; i++) {
            ...
            if (style === 0) {
              ...
              speed = Math.random() * 5 + 2; // 速度稍快，更有张力
              decay = Math.random() * 0.009 + 0.006; // 衰减更慢，持续更久
              size = 2.5; // 粒子更大
            ...
            // 样式2：心形扩散（更亮的粉色系）
              ...
              speed = (Math.random() * 4 + 1) * (r / 16 + 0.5);
              decay = Math.random() * 0.008 + 0.007;
              size = 2.2;
              color = `hsl(${340 + Math.random() * 20}, 100%, 80%)`; // 亮度提升到80%
            }
            // 样式3：螺旋形扩散（替换十字形，自然绚烂）
            else if (style === 2) {
              // 螺旋角度计算，更自然的轨迹
              ...
              size = 2.0;
              color = `hsl(${200 + Math.random() * 30}, 95%, 75%)`; // 亮蓝色系
            }
            // 样式4：环形扩散（双层，更亮的黄色系）
            else if (style === 3) {
              angle = Math.random() * Math.PI * 2;
              ...
              size = 2.0;
              color = `hsl(${60 + Math.random() * 30}, 100%, 80%)`; // 亮度提升到80%
              // 额外生成内层粒子，更浓密
              if (i % 2 === 0) {
                particles.push(
                  new Particle(
            ...
            // 样式5：不规则散射（全色系高亮）
              ...
              decay = Math.random() * 0.007 + 0.004;
              size = 2.0;
              color = `hsl(${Math.random() * 360}, 95%, 80%)`; // 全色系亮度80%
            }

            particles.push(
              new Particle(this.x, this.y, angle, speed, decay, size, color),
            );
          }
        }
      }
      ...
      // 烟花粒子类（优化亮度和持续时间）
      class Particle {
        constructor(x, y, angle, speed, decay, size, color) {
          this.x = x;
          this.y = y;
          this.angle = angle;
          this.speed = speed;
          this.gravity = 0.01; // 重力更弱，下落更慢
          this.vx = Math.cos(this.angle) * this.speed;
          this.vy = Math.sin(this.angle) * this.speed;
          this.alpha = 1;
          this.decay = decay; // 更慢的衰减
          this.size = size;
          this.color = color;
        }
